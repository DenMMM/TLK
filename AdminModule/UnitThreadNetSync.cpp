...
//---------------------------------------------------------------------------
bool MThreadNetSync::Send(char *Buffer_, int Size_)
{
    int result;

    while(Size_)
    {
        result=::send(RemoteSocket,Buffer_,Size_,0);
...
void MThreadNetSync::Process(MComputer *Computer_)
{
    ...
    if ( (SyncData&mnsState)||(!(SyncData&mnsGames)) )
    {
        ...
        StartWaiting(10);
        result=Send((char*)&data_size,sizeof(int));
        StopWaiting();
        if ( !result ) goto error;
        // Отправляем данные
        StartWaiting(30);
        result=Send(data,data_size);
        StopWaiting();
        if ( !result ) goto error;
...
char *MThreadNetSync::SetComputerData(char *Data_, MComputer *Computer_)
{
    ...
    *((unsigned int*)Data_)=Computer_->StateVer; Data_+=sizeof(unsigned int);
